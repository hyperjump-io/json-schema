...
export const serialize = (compiledSchema) => {
  // Convert known plugin locations to marker objects so JSON.stringify
  // doesn't have to pattern-match plugin shapes everywhere.
...
export const deserialize = (serialized, options = {}) => {
  return JSON.parse(serialized, (_key, value) => {
    ...
    if (value.__type === "PluginSet") {
      // Legacy PluginSet: restore to a Set of plugin objects
      return new Set((value.values || []).map((p) => resolvePlugin(p.id, options)));
    }
