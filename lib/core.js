...
import { JsInstance } from "./instance.js";
...
export const validate = async (url, value = undefined, outputFormat = undefined) => {
  ...
  const interpretAst = (value, outputFormat) => interpret(compiled, new JsInstance(value), outputFormat);
...
subscribe("validate.metaValidate", async (_message, schema) => {
  if (getShouldValidateSchema() && !schema.document.validated) {
    ...
    // Interpret
    const schemaInstance = new JsInstance(schema.document.root, schema.document.baseUri);
