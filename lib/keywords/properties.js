...
const interpret = (properties, instance, ast, dynamicAnchors, quiet) => {
  return instance.typeOf() !== "object" || pipe(
    instance.entries(),
    filter(([propertyName]) => propertyName.value() in properties),
    every(([propertyName, property]) => Validation.interpret(properties[propertyName.value()], property, ast, dynamicAnchors, quiet))
  );
};
