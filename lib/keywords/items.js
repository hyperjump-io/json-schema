...
const interpret = (items, json, ast) => {
  ...
  if (typeof items === "string") {
    return Json.every((itemValue) => Core.interpretSchema(items, itemValue, ast), json);
  } else {
    return Json.every((item, ndx) => !(ndx in items) || Core.interpretSchema(items[ndx], item, ast), json);
  }
};
